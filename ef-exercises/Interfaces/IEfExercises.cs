...
public interface IEfExercises
{
    ...
    double GetTotalSalaryByDepartment(string departmentName);
    ...
    List<Employee> GetEmployeesWithSalaryAbove(double minSalary);
    ...
    Department GetDepartmentWithHighestBudget();

    // Level 2 Exercises - Slightly harder queries

    List<Employee> GetEmployeesHiredBetween(DateTime startDate, DateTime endDate);
    ...
    List<Employee> GetTopNHighestPaidEmployees(int count);
    ...
    List<Department> GetDepartmentsWithAverageSalaryAbove(double minAverageSalary);
}